...
#define tx_enable_interrupt     0x8000C      //for bar1 transmit enable interrupt
#define tx_data                 0x80004         //for bar1 tx data
...
int main(int argc, char **argv){
    ...
    printf("SLEEP FOR %4ld microsecond \n", delayValue2);                              //time sleep
    usleep(delayValue2);
    ...
    // converted = write_value+'0';

    if(write_value == chr){
            printf("PASSSED and Value Received is  - %c", write_value);
        } else {
            printf("FAILED and value received is  - %c", write_value);
        }
